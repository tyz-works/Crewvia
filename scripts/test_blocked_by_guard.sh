...
echo "== test_blocked_by_guard.sh (blocked_by defense-in-depth + BC-1/BC-2 fix) =="
...
write_task() {
  local id="$1" title="$2" status="$3" blocked_by="$4"
  local blocked_yaml
  ...
  printf -- '---\nid: %s\ntitle: %s\nskills: [bash]\npriority: medium\nstatus: %s\nblocked_by: %s\nworker: null\nstarted_at: null\ncompleted_at: null\n---\n\n## Description\nTask %s.\n\n## Result\n' \
    "$id" "$title" "$status" "$blocked_yaml" "$id" > "$TASKS_DIR/${id}.md"
}

reset_task() {
  # Reset a task back to pending
  write_task "$1" "$2" "pending" "$3"
}
...
write_task t002 "Second task (pending, no deps)" pending ""

# t003: pending, blocked by t001 (done) → unblocked
write_task t003 "Third task (blocked_by t001=done)" pending "t001"

# t004: pending, blocked by t002 (pending) → still blocked
write_task t004 "Fourth task (blocked_by t002=pending)" pending "t002"

# t005: pending, blocked by t006 (verified)
write_task t005 "Task blocked by verified dep" pending "t006"
...
reset_task t003 "Third task (blocked_by t001=done)" "t001"
...
reset_task t005 "Task blocked by verified dep" "t006"
