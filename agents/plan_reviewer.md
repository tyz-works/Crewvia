...
ただし **approve だけは、`plan_review.md` 1 行目の正規の
`**Verdict:** approve` と構造化出力の approve が両方そろわないと成立しない**
(t020。下記参照)。
...
- **`queue/missions/<slug>/plan_review.md` の 1 行目 (ファイル先頭の最初の
  ...
  (`**Verdict:** approve (軽微な指摘あり)` は**書式違反**として扱われ、
   差し戻される。註釈は次の行以降に書くこと。)
- **1 行目に正規の `**Verdict:** approve` が無いと approve は成立しない。**
  構造化出力が approve でも、`plan_review.md` の判定行を書かなかった・書式を
  外した場合は自動判定されず、Director の手動確認になる (review cycle は
  消費されない)。
...
- `plan_review.md` に verdict 行を**一切**書かなかった場合 (別表記の
  「総合判定: 要修正」だけ、など) に限り、構造化出力が **revise / reject** なら
  `review-plan.sh` がその値を取り、`plan_review.md` の 1 行目に規定形式で書き戻す。
  構造化出力が approve の場合は書き戻さず、手動確認になる (t020)。
