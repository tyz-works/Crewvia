...
**デフォルト動作**
- 全ての名前がOrchestratorにもWorkerにもなれる
...
customizations:
  - name: Kai
    role: orchestrator    # Orchestrator固定
...
/
  ...
  agents/
    orchestrator.md     Orchestratorのシステムプロンプト
...
```
自律実行OK（承認不要）    → Orchestratorに提案 → Backlogに自動追加
...
autonomous-improvement.yaml の基準で自己判断
   ↓
自律実行OK → Orchestratorに「改善提案: xxx」を報告
             Orchestratorがカードを作成してBacklogに積む
             優先度低めで自動実行
   ↓
...
3. OK → Orchestratorに「改善提案」として報告（自分でタスク化しない）
4. NG → `type: improvement` でTaskvia `/api/log` に投稿して終了
5. OrchestratorがBacklogに積むかどうかを判断する

※ Workerが自分でBacklogにカードを追加しないこと。必ずOrchestratorを経由する。
