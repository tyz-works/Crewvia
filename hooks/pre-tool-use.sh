...
#
# t002 (#183, 方針の根拠は Result 参照) での修正: 「クォート除去後も制御演算子
# (`;`/`&&`/`||`/`|`) が残るなら常に元の $COMMAND (クォート込み) で判定する」
# というフェイルセーフは、報告コマンドに後続パイプが付くだけで t019 の誤 deny
# バグを再発させていた (例: `./scripts/plan.sh done t001 "...cat >> .../t004.md
# を引用した説明..." | tee log` — 末尾の `| tee log` という**本物の**制御演算子
# のせいでフェイルセーフに落ち、クォート込みの生コマンドを grep すると引用文中の
# `cat >> .../t004.md` が再び「本物のリダイレクト」として誤検出される)。
#
# 制御演算子が残ること自体は「クォートの外側に本物のシェル構文がある」ことしか
# 意味せず、その本物の構文は (クォート内でない限り) 除去後の文字列にもそのまま
# 残っているため、フェイルセーフは実際には不要 (`X; ./scripts/plan.sh --help;
# cat >> .../t001.md <<EOF` の実書き込みはクォートで囲まれていないので除去後
# 文字列でも検出できる)。フェイルセーフが本当に必要なのは、クォートの中身が
# **ネストしたインタプリタに丸ごと渡されて実際に実行される**場合だけ (例:
# `bash -c "cat >> .../t004.md"; true` — この場合にクォートを除去すると本物の
# 書き込みを見逃す)。そこで「制御演算子が残る」ことをフェイルセーフの唯一の
# トリガーにするのをやめ、上記のようなネストインタプリタ呼び出しを示す狭い
# マーカーが実際に含まれる場合だけ元の $COMMAND で判定するよう絞り込む。
#
# ★ このマーカー一覧は「書き込みを検出する」denylist (リダイレクト/sed -i/tee
#   の 3 パターン) を拡張するものではない。あくまで「クォート除去後の文字列を
#   信用してよいか」を決める別レイヤの判定であり、網羅は主張しない。ここに無い
#   方法 (未知のラッパースクリプト等) でネストインタプリタに渡された場合は
#   引き続き検出できない — これは変数展開 / dd / python -c 等、既存の
#   「既知の残存リスク」と同じ性質の残存リスクとして扱う (worker.md の明文化が
#   一次防御である前提は変わらない)。
  ...
  case "$COMMAND" in
    ...
    *)
      ...
      case "$_CMD_NO_QUOTES" in
        *';'*|*'&&'*|*'||'*|*'|'*)
          # 制御演算子が残る (=元からクォート外) だけでは生コマンドに戻さない
          # (t002/#183)。ネストインタプリタへの丸投げを示す狭いマーカーが
          # 実際にある場合に限り、フェイルセーフとして元の $COMMAND で判定する。
          _QM_NESTED_INTERP_RE='\b(bash|sh|zsh|dash|ksh)[[:space:]]+-c\b|\bpython3?[[:space:]]+-c\b|\b(perl|node|ruby)[[:space:]]+-e\b|\beval\b'
          if ! printf '%s' "$COMMAND" | grep -qE "$_QM_NESTED_INTERP_RE"; then
